...
static void copy_buffer(CC_Deque const *const deque, void **buff,
                        void *(*cp)(void *));
 ...
 *
 * The CC_Deque is allocated using the allocators specified in the CC_DequeConf
 * struct. The allocation may fail if the underlying allocator fails.
 *
 * @param[in] conf CC_Deque configuration structure. All fields must be
 ...
 *
 * @note
 * This function should not be called on a CC_Deque that has some of its
...
enum cc_stat cc_deque_add_at(CC_Deque *deque, void *element, size_t index) {
    if (index >= deque->size) return CC_ERR_OUT_OF_RANGE;
    ...
    if (index == 0) return cc_deque_add_first(deque, element);

    if (index == c) return cc_deque_add_last(deque, element);
    ...
    if (index <= (deque->size / 2) - 1) {
        if (p < f || f == 0) {
            ...
            if (f != 0) {
                memmove(&(deque->buffer[f - 1]), &(deque->buffer[f]),
                        r_move * sizeof(void *));
            }
            if (p != 0) {
                memmove(&(deque->buffer[0]), &(deque->buffer[1]),
                        l_move * sizeof(void *));
            }
        ...
        if (p > l || l == c) {
            ...
            if (p != c) {
                memmove(&(deque->buffer[p + 1]), &(deque->buffer[p]),
                        (c - p) * sizeof(void *));
            }
            if (l != c) {
                memmove(&(deque->buffer[1]), &(deque->buffer[0]),
                        (l + 1) * sizeof(void *));
            }
 ...
 * @param[in] index the index at which the replacement element should be
 * inserted
 * @param[out] out     Pointer to where the replaced element is stored, or NULL
 * if it is to be ignored
 *
 * @return CC_OK if the element was successfully replaced, or
 * CC_ERR_OUT_OF_RANGE if the index was out of range.
 */
enum cc_stat cc_deque_replace_at(CC_Deque *deque, void *element, size_t index,
                                 void **out) {
    if (index >= deque->size) return CC_ERR_OUT_OF_RANGE;

    size_t i = (deque->first + index) & (deque->capacity - 1);

    if (out) *out = deque->buffer[i];
...
enum cc_stat cc_deque_remove_at(CC_Deque *deque, size_t index, void **out) {
    ...
    if (index == 0) return cc_deque_remove_first(deque, out);

    if (index == c) return cc_deque_remove_last(deque, out);
    ...
    if (index <= (deque->size / 2) - 1) {
        ...
        if (p > l) {
            ...
            if (p != c) {
                memmove(&(deque->buffer[p]), &(deque->buffer[p + 1]),
                        (c - p) * sizeof(void *));
            }
            if (p != 0) {
                memmove(&(deque->buffer[1]), &(deque->buffer[0]),
 ...
 * @param[out] out Pointer to where the removed value is stored, or NULL if it
 * is to be ignored
 *
 * @return CC_OK if the element was successfully removed, or CC_ERR_OUT_OF_RANGE
...
enum cc_stat cc_deque_remove_first(CC_Deque *deque, void **out) {
    if (deque->size == 0) return CC_ERR_OUT_OF_RANGE;
 ...
 * was out of range.
 */
enum cc_stat cc_deque_get_at(CC_Deque const *const deque, size_t index,
                             void **out) {
    if (index > deque->size) return CC_ERR_OUT_OF_RANGE;
 ...
 */
enum cc_stat cc_deque_copy_shallow(CC_Deque const *const deque,
                                   CC_Deque **out) {
    CC_Deque *copy = deque->mem_alloc(sizeof(CC_Deque));

    if (!copy) return CC_ERR_ALLOC;
 ...
 * memory allocation for the copy failed.
 */
enum cc_stat cc_deque_copy_deep(CC_Deque const *const deque,
                                void *(*cp)(void *), CC_Deque **out) {
    CC_Deque *copy = deque->mem_alloc(sizeof(CC_Deque));
...
enum cc_stat cc_deque_trim_capacity(CC_Deque *deque) {
    if (deque->capacity == deque->size) return CC_OK;

    size_t new_size = upper_pow_two(deque->size);

    if (new_size == deque->capacity) return CC_OK;

    void **new_buff = deque->mem_alloc(sizeof(void *) * new_size);

    if (!new_buff) return CC_ERR_ALLOC;
...
/**
 * Returns the number of occurrences of the element within the specified
 * CC_Deque.
 *
...
size_t cc_deque_contains(CC_Deque const *const deque, const void *element) {
    ...
    for (i = 0; i < deque->size; i++) {
        ...
        if (deque->buffer[p] == element) o++;
    }
    return o;
}

/**
 * Returns the number of occurrences of the value poined to by
 * <code>element</code> within the deque.
 *
 * @param[in] deque CC_Deque that is being searched
 * @param[in] element the element that is being searched for
 ...
 * @return the number of occurrences of the element
 */
size_t cc_deque_contains_value(CC_Deque const *const deque, const void *element,
    ...
    for (i = 0; i < deque->size; i++) {
        size_t p = (deque->first + i) & (deque->capacity - 1);
        if (cmp(deque->buffer[p], element) == 0) o++;
    }
    return o;
}
 ...
 */
enum cc_stat cc_deque_index_of(CC_Deque const *const deque, const void *element,
                               size_t *index) {
    size_t i;
...
enum cc_stat cc_deque_filter_mut(CC_Deque *deque, bool (*pred)(const void *)) {
    if (cc_deque_size(deque) == 0) return CC_ERR_OUT_OF_RANGE;
...
/**
 * Filters the CC_Deque by creating a new CC_Deque that contains all elements
 * from the original CC_Deque that return true on pred(element) without
 * modifying the original deque.
 *
 * @param[in] deque deque that is to be filtered
 ...
 * new deque failed.
 */
enum cc_stat cc_deque_filter(CC_Deque *deque, bool (*pred)(const void *),
                             CC_Deque **out) {
    if (cc_deque_size(deque) == 0) return CC_ERR_OUT_OF_RANGE;
    ...
    if (!filtered) return CC_ERR_ALLOC;
...
/**
 * Copies the elements from the CC_Deque's buffer to the buffer buff. This
 * function only copies the elements instead of the whole buffer and also
 * realigns the buffer in the process.
 *
 * @param[in] deque The deque whose buffer is being copied to the new buffer
 * @param[in] buff The destination buffer. This buffer is expected to be at
 * least the capacity of the original deque's buffer or greater.
 * @param[in] cp An optional copy function that returns a copy of the element
 * passed to it. If NULL is passed, then only a shallow copy will be performed.
 */
static void copy_buffer(CC_Deque const *const deque, void **buff,
                        void *(*cp)(void *)) {
    if (cp == NULL) {
        if (deque->last > deque->first) {
            memcpy(buff, &(deque->buffer[deque->first]),
...
static INLINE size_t upper_pow_two(size_t n) {
    if (n >= MAX_POW_TWO) return MAX_POW_TWO;
...
enum cc_stat cc_deque_iter_next(CC_DequeIter *iter, void **out) {
    ...
    if (last == first || iter->index >= iter->deque->size) return CC_ITER_END;
...
enum cc_stat cc_deque_iter_remove(CC_DequeIter *iter, void **out) {
    if (iter->last_removed) return CC_ERR_VALUE_NOT_FOUND;
 ...
 */
enum cc_stat cc_deque_iter_replace(CC_DequeIter *iter, void *replacement,
                                   void **out) {
 ...
 *
 * @note
 * This function should not be called before a call to
 * <code>cc_deque_iter_next()
 * </code>
 *
 * @param[in] iter the iterator on which this operation is being performed
 *
 * @return the index
 */
size_t cc_deque_iter_index(CC_DequeIter *iter) { return iter->index - 1; }
 ...
 * of one of the deques has been reached.
 */
enum cc_stat cc_deque_zip_iter_next(CC_DequeZipIter *iter, void **out1,
                                    void **out2) {
 ...
 * <code>cc_deque_zip_iter_next()</code> without invalidating the iterator.
 *
...
enum cc_stat cc_deque_zip_iter_add(CC_DequeZipIter *iter, void *e1, void *e2) {
    ...
    /* While this check is performed by a call to cc_deque_add_at, it is
       necessary to know in advance whether both deque buffers have enough room
       before inserting new elements because this operation must insert either
       both elements, or none.*/
    if ((iter->d1->capacity == iter->d1->size &&
         expand_capacity(iter->d1) != CC_OK) &&
        (iter->d2->capacity == iter->d2->size &&
         expand_capacity(iter->d2) != CC_OK)) {
        return CC_ERR_ALLOC;
    }
...
/**
 * Removes and outputs the last returned element pair by
 * <code>cc_deque_zip_iter_next()
 ...
 *
 * @return CC_OK if the element was successfully removed, CC_ERR_OUT_OF_RANGE if
 * the iterator is in an invalid state, or CC_ERR_VALUE_NOT_FOUND if the value
 * was already removed.
 */
enum cc_stat cc_deque_zip_iter_remove(CC_DequeZipIter *iter, void **out1,
                                      void **out2) {
    if (iter->last_removed) return CC_ERR_VALUE_NOT_FOUND;

    if ((iter->index - 1) >= iter->d1->size ||
        (iter->index - 1) >= iter->d2->size)
        return CC_ERR_OUT_OF_RANGE;
...
/**
 * Replaces the last returned element pair by
 * <code>cc_deque_zip_iter_next()</code> with the specified replacement element
 * pair.
 *
 * @param[in] iter  Iterator on which this operation is being performed
 ...
 * CC_ERR_OUT_OF_RANGE.
 */
enum cc_stat cc_deque_zip_iter_replace(CC_DequeZipIter *iter, void *e1,
                                       void *e2, void **out1, void **out2) {
    if ((iter->index - 1) >= iter->d1->size ||
        (iter->index - 1) >= iter->d2->size)
        return CC_ERR_OUT_OF_RANGE;
...
/**
 * Returns the index of the last returned element pair by
 * <code>cc_deque_zip_iter_next()</code>.
 *
 * @param[in] iter Iterator on which this operation is being performed
 *
